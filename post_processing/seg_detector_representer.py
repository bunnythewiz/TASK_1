    ...
    def __call__(self, batch, pred, is_output_polygon=False):
        ...
        for batch_index in range(pred.size(0)):
            ...
            if isinstance(shape, torch.Tensor):
                ...
                if shape.dim() == 0:  # Scalar tensor
                    height = width = shape.item()
                elif shape.dim() == 1:
                    if len(shape) == 2:  # [H, W] tensor
                        height, width = shape[0].item(), shape[1].item()
                    elif len(shape) == 1:  # Single value tensor [size]
                        height = width = shape[0].item()
                    else:
                        raise ValueError(f"Unexpected shape tensor length: {len(shape)}")
                else:
                    raise ValueError(f"Unexpected shape tensor dimensions: {shape.dim()}")
